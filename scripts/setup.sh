...
# ── Форк апстрима + автообновление ──────────────────────────────────────────
# Railway передеплоит по push В ПОДКЛЮЧЁННЫЙ репозиторий. Подключить апстрим
# напрямую нельзя — чужие пуши не триггерят ТВОЙ деплой (это и был баг:
# инстансы «замерзали»). Поэтому подключаем ТВОЙ ФОРК: workflow
# sync-upstream.yml в форке каждые ~5 минут подтягивает апстрим fast-forward'ом,
# Railway видит push в форк и пересобирает.
CONNECT_REPO="$REPO"   # безопасный фолбэк: прямое подключение апстрима

# gh залогинен? Если gh есть, но входа нет — предложим войти (в интерактиве).
if command -v gh &>/dev/null && ! gh auth status &>/dev/null && [[ -t 0 ]]; then
  echo "Нужен вход в GitHub (gh) — для форка и автообновления."
  gh auth login || true
fi
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [[ -n "$GH_USER" ]]; then
    echo "Форкаю $REPO → $GH_USER (идемпотентно)..."
    gh repo fork "$REPO" --clone=false >>"$LOG" 2>&1 || true
    # Форк создаётся асинхронно — ждём, пока появится.
    for _ in 1 2 3 4 5 6; do
      gh repo view "$GH_USER/tg-ai-assistant" &>/dev/null && break
      sleep 5
    done
  ...
  echo -e "${YELLOW}gh CLI не найден/не залогинен — форкни через веб:${RESET}"
  echo "  1) Открой https://github.com/$REPO/fork и нажми «Create fork»."
  echo "  2) В форке: вкладка Actions → включи workflows (кнопка «I understand…»)."
  if [[ -t 0 ]]; then
    ask "Введи свой GitHub-логин (или Enter — подключить апстрим напрямую):"
    read -r GH_USER_INPUT
    [[ -n "$GH_USER_INPUT" ]] && CONNECT_REPO="$GH_USER_INPUT/tg-ai-assistant"
  fi
fi

# Подключаем источник ВСЕГДА (прошлая неудачная попытка могла оставить пустой сервис).
echo "Подключаю код из $CONNECT_REPO (Railway передеплоит при пушах в него)..."
STEP_OUT=$(mktemp)
if ! railway service source connect --repo "$CONNECT_REPO" --branch main --service "$SERVICE" --json >"$STEP_OUT" 2>&1; then
...
echo ""
echo -e "${YELLOW}Обновления прилетают сами:${RESET} сервис подключён к $CONNECT_REPO (ветка main)."
if [[ "$CONNECT_REPO" != "$REPO" ]]; then
  echo "Твой форк каждые ~5 минут подтягивает апстрим ($REPO) и Railway пересобирает."
else
  echo -e "${YELLOW}Подключён апстрим напрямую — автообновление не сработает.${RESET}"
  echo "Форкни $REPO, включи Actions и переключи Source на форк, чтобы обновляться само."
fi
echo "Твои данные/ключи это не трогает."
